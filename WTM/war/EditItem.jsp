...
<body>
	...
	<form name="frmCreateItem" onSubmit="validate()" action="addEdit" method="post">
		<table align="center">
			<tr>
				<td>Task Name:</td>
				<td><input name="taskname" type="text" value=<%=item.getName()%> /></td>
			</tr>
			<tr>
				<td>Note:</td>
			...
			<tr>
				<td>Due Time:</td>
				<td><input name="timepicker" id="timepicker" value=<%=item.getDueTime2()%> /> <br/></td> 
			</tr>
			<tr>
				<td>Priority:</td>
				<td>
					<input type="radio" name="priority" id="low" value="1" <%if(item.getPriority() == 1) {%> checked <%}%> /><label for="low" /></label>
						<img src="../Resources/circle_green.png" height="20">
					<input type="radio" name="priority" id="medium" value="2" <%if(item.getPriority() == 2) {%> checked <%}%>/><label for="medium" /></label>
						<img src="../Resources/circle_yellow.png" height="20">
					<input type="radio" name="priority" id="high" value="3" <%if(item.getPriority() == 3) {%> checked <%}%>/><label for="high" /></label>
